...
func messageHandler(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if command == "search" {
		...
		res, err := dl.Run(context.TODO(), fmt.Sprintf("ytsearch1:%v", strings.Join(args[1:], " ")), "--get-json")

		fmt.Println("Info", res.Stdout)

		if err != nil {
			fmt.Println("Error searching for the songs: ", err)
			return
		}

		_, err = s.ChannelMessageSendReply(m.ChannelID, fmt.Sprintf("%v", res.Stdout), m.Reference())
